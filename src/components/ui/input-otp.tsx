...
const InputOTPSlot = React.forwardRef<React.ElementRef<"div">, React.ComponentPropsWithoutRef<"div"> & { index: number }>(({ index, className, ...props }, ref) => {
	...
	return (
		<div
			ref={ref}
			className={cn(
				"relative flex h-10 w-10 items-center justify-center border border-zinc-200 bg-white text-base ring-offset-white transition-all first:rounded-l-md first:border-l last:rounded-r-md dark:border-zinc-800 dark:bg-zinc-950 dark:ring-offset-zinc-950",
				isActive && "z-10 ring-1 ring-zinc-500 ring-offset-2 dark:ring-zinc-300",
				className
			)}
			{...props}
		>
